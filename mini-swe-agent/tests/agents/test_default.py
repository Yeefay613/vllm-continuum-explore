...
def test_successful_completion():
    ...
    assert len(
        agent.messages) == 6  # system, user, assistant, user, assistant, user
...
def test_format_error_handling():
    ...
    assert (len([
        ...
        if "Please always provide EXACTLY ONE action" in msg.get(
            "content", "")
    ]) == 2)
...
def test_timeout_captures_partial_output():
    ...
    assert expected_output in timed_out_messages[0][
        "content"]  # ensure timed out output is still captured
...
def test_multiple_steps_before_completion():
    ...
    agent = DefaultAgent(
        ...
        env=LocalEnvironment(),
        cost_limit=
        5.0,  # Increase cost limit to allow all 4 calls (4.0 total cost)
    )
