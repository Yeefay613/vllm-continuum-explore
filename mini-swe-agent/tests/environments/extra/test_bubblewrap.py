...
from minisweagent.environments.extra.bubblewrap import (
    BubblewrapEnvironment, BubblewrapEnvironmentConfig)


@pytest.mark.skipif(not shutil.which("bwrap"),
...
def test_bubblewrap_environment_cwd_parameter_override():
    ...
    ) as temp_dir1, tempfile.TemporaryDirectory() as temp_dir2:
...
def test_bubblewrap_environment_nonexistent_command():
    ...
    try:
        ...
        print(
            f"test_bubblewrap_environment_nonexistent_command result: {result}"
        )
        assert result["returncode"] != 0
        assert "nonexistent_command_12345" in result[
            "output"] or "command not found" in result["output"]
    finally:
        env.cleanup()
