...
class TestGetModelName:
    ...
    def test_input_model_name_takes_precedence(self):
        ...
        with patch.dict(os.environ, {"MSWEA_MODEL_NAME": "env-model"}):
            assert get_model_name("input-model",
                                  self.CONFIG_WITH_MODEL_NAME) == "input-model"
    ...
    def test_config_takes_precedence_over_env(self):
        ...
        with patch.dict(os.environ, {"MSWEA_MODEL_NAME": "env-model"}):
            assert get_model_name(
                None, self.CONFIG_WITH_MODEL_NAME) == "config-model"
...
class TestGetModel:
    ...
    def test_get_deterministic_model(self):
        ...
        model = get_model("test-model",
                          config | {"model_class": "deterministic"})
...
class TestGlobalModelStats:
    ...
    def test_prints_cost_limit_when_set(self, capsys):
        ...
        with patch.dict(os.environ, {"MSWEA_GLOBAL_COST_LIMIT": "5.5"},
                        clear=True):
            GlobalModelStats()
    ...
    def test_prints_call_limit_when_set(self, capsys):
        ...
        with patch.dict(os.environ, {"MSWEA_GLOBAL_CALL_LIMIT": "10"},
                        clear=True):
            GlobalModelStats()
            captured = capsys.readouterr()
    ...
    def test_prints_both_limits_when_both_set(self, capsys):
        ...
        with patch.dict(os.environ, {
                "MSWEA_GLOBAL_COST_LIMIT": "2.5",
                "MSWEA_GLOBAL_CALL_LIMIT": "5"
        },
                        clear=True):
            GlobalModelStats()
            captured = capsys.readouterr()
